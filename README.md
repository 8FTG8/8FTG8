...
**Também no dia a dia:** Riverpod, Provider & BloC (gerenciamento de estado) · GoRouter · Cloud Functions v2 · Cloud Firestore & Storage · Firestore/Storage Security Rules · SQFlite (offline-first) · Jest & Firebase Rules Unit Testing · integrações de API (ASAAS, Z-API/WhatsApp)
...
**[Aplicativo EggGo](https://github.com/8FTG8/EggGo)** `(freelance · em produção)` — Projeto freelance para uma distribuidora de alimentos, prototipado no Figma e construído com Clean Architecture (MVC-adaptado) antes de codar. App em Flutter para gerenciamento de clientes, produtos e vendas, criado sob medida para a empresa [EggGo](https://www.instagram.com/egg.go_ovos). Funciona **offline-first** (SQFlite), sincronizando automaticamente com o Firestore assim que a conexão volta, com exportação de comprovantes em PDF e dashboard de vendas filtrável por período. Gerenciamento de estado com Provider.

**HealthCaree** `(em desenvolvimento)` — SaaS multi-tenant de gestão para clínicas médicas e multiespecialidade, com suporte a múltiplos profissionais por clínica. Flutter + Firebase: agenda, cadastro de pacientes, controle financeiro e relatórios exportáveis em PDF, com interface desktop-first responsiva construída sobre Material Design 3 e um design system próprio (tokens de espaçamento, raio e sombra).
...
**Data Analyst & Python Developer** `(estágio)` — Automação de processos internos e scripts de web scraping para coleta de dados públicos, dando suporte à análise de dados para decisões estratégicas. Desenvolvi também o site institucional do Departamento de Genética da FMRP-USP em WordPress e estruturei a plataforma Moodle Cloud do departamento.

**Freelance Web Developer** — Sites institucionais e e-commerce sob demanda em WordPress/HTML/CSS/PHP, e customização de lojas e sites em ferramentas low-code/no-code (Shopify, GreatPages) para múltiplos clientes de diferentes segmentos.